...
def llm_find_best_from_top3(a_text, top3_data, api_key, log_func=print):
    ...
    prompt = f"""##角色:你是物料匹配专家
##任务:从"##B候选"的3条物料中找出与"##A材料"最相似的1条，并给出相似度评分
##A材料: {a_text}
##B候选:
{b_list_str}
##匹配标准:
- 只有名称/规格/尺寸中有一项语义非常相近才匹配
- 必须给出一个相似度评分(0-100整数)
...
"""
    ...
    try:
        ...
        log_func(f"[LLM请求] Prompt:\n{prompt[:500]}..." if len(prompt) > 500 else f"[LLM请求] Prompt:\n{prompt}")
...
class AISemanticMatchApp:
    ...
    def load_config(self):
        if os.path.exists(CONFIG_FILE):
            try:
                ...
                self.sim_top1_th = cfg.get("sim_top1_th", self.sim_top1_th if hasattr(self, 'sim_top1_th') else 0.92)
                self.diff_top12_th = cfg.get("diff_top12_th", self.diff_top12_th if hasattr(self, 'diff_top12_th') else 0.04)
            except:
                pass
    ...
    def run_match(self):
        ...
        for idx, a_txt in enumerate(a_list):
            ...
            gate_pass = (len(top3_data) >= 1) and (isinstance(top1_val, float) and math.isfinite(top1_val)) and \
                        (top1_val >= self.sim_top1_th) and ((top1_val - top2_val) >= self.diff_top12_th)
            # Log gate decision details for debugging
            log_print(self.log_box, f"Gate decision: PASS={gate_pass}, top1={top1_val:.3f}, top2={top2_val:.3f}, diff={top1_val-top2_val:.3f}, candidates={len(top3_data)}")
            if gate_pass:
                first = top3_data[0]
                code = first[0]
                name = first[1]
                ...
                result.append([a_txt, name, spec + size_str, code, ai_score, "已匹配"])
                log_print(self.log_box, f"Stage1: 直接匹配成功：{name} ({code}) 评分:{ai_score}")
                match_count += 1
            else:
                log_print(self.log_box, f"→ 第{idx+1}/{total}条：向量化相似度={top1_val:.3f}，Top2={top2_val:.3f}，正在LLM选优...")
            # Suppress detailed LL.M requests/responses in logs: only keep final score/logical outcome
            result_tuple = llm_find_best_from_top3(a_txt, top3_data, api_key, log_func=lambda msg: "LLM处理中")
                best_idx, code, name, spec, size, ai_score = result_tuple
                
                if best_idx >= 0 and ai_score >= 60:
                    size_str = " " + size if size else ""
                    result.append([a_txt, name, spec + size_str, code, ai_score, "已匹配"])
                    match_count += 1
                    log_print(self.log_box, f"  ✅ 匹配成功：{name} ({code}) 评分:{ai_score}")
                else:
                    result.append([a_txt, "", "", "", 0, "未达标"])
                    log_print(self.log_box, f"  ❌ 未达标(AI评分:{ai_score})")
        ...
        ws_out.append(["A表内容", "B表名称", "B表规格", "B表编码", "AI得分", "状态"])
        for r in result:
            ws_out.append(r)
    ...
    def create_widgets(self):
        ...
        frame_step1.place(x=20, y=90, width=860, height=130)
        ...
        self.cache_combo = ttk.Combobox(frame_step1, width=35, state="readonly")
        self.cache_combo.place(x=80, y=83)
        self.cache_combo.bind('<<ComboboxSelected>>', self.on_cache_selected)

        # 批量大小和并发数
        ttk.Label(frame_step1, text="批量大小：").place(x=10, y=85)
        ttk.Combobox(frame_step1, textvariable=self.batch_size, values=[10, 20, 40], width=6, state="readonly").place(x=75, y=83)

        ttk.Label(frame_step1, text="并发数：").place(x=160, y=85)
        ttk.Combobox(frame_step1, textvariable=self.max_concurrency, values=[1, 2, 4, 8], width=6, state="readonly").place(x=220, y=83)

        # 向量化进度条
        ttk.Label(frame_step1, text="向量化进度：").place(x=320, y=85)
        self.progress = ttk.Progressbar(frame_step1, mode="determinate", length=200)
        self.progress.place(x=410, y=83, width=180, height=18)

        # 向量化按钮（开始/停止切换）
        self.vectorize_btn = ttk.Button(frame_step1, text="开始向量化", command=self.toggle_vectorization, width=15)
        self.vectorize_btn.place(x=700, y=78)

        # ==================== 第二步：A表处理区 ====================
        frame_step2 = ttk.LabelFrame(self.root, text="第二步：A表处理（执行匹配）")
        frame_step2.place(x=20, y=230, width=860, height=130)
        ...
        self.match_btn.place(x=10, y=55)
        ttk.Button(frame_step2, text="停止运行", command=self.stop_match, width=15).place(x=160, y=55)

        # 提示
        ttk.Label(frame_step2, text="⚠️ 请先完成B表向量化后再执行匹配", foreground="gray").place(x=350, y=58)
        ...
        frame_step3.place(x=20, y=370, width=860, height=320)

        # 运行日志
        self.log_box = tk.Text(frame_step3, state=tk.DISABLED, font=("微软雅黑", 9))
        self.log_box.place(x=10, y=10, width=840, height=250)

        # 滚动条
        scrollbar = ttk.Scrollbar(frame_step3, command=self.log_box.yview)
        scrollbar.place(x=850, y=10, height=250)
        self.log_box.config(yscrollcommand=scrollbar.set)

        # 打开结果按钮
        ttk.Button(frame_step3, text="打开结果文件", command=self.open_result_file, width=15).place(x=10, y=270)
